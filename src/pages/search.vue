      ...
      if(this.isLogin){
        ...
        let header={
          headers:{
            ...
            "sign":md5("/home/getProdSearchList"+this.$store.state.loginUser.token+JSON.stringify(new Date().getTime())).toUpperCase()
          }
        }
        // 获取历史搜索
    ...
    goSearch:function(){
      if(this.isLogin){
        let header={
          "token":this.$store.state.loginUser.token,
          "time":JSON.stringify(new Date().getTime()),
          "sign":md5("/home/insertProdSearch"+this.$store.state.loginUser.token+JSON.stringify(new Date().getTime())).toUpperCase()
        }
        // 设置传值
        let cartData={
