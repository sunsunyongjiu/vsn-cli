		...
		<div class="vsn-main">
            ...
            <swiper-item class="swiper-demo-img" v-for="(item, index) in detailObj.topImg"  :key="index"><img :src="item.file_path"></swiper-item>
					...
					<div class="detail-title">
						...
						<div class="font-12 color-91 stocks" v-if="detailObj.sellType==1">
							库存：{{detailObj.stocks}}
						</div>
						...
						<popup v-model="popShow" position="bottom" height="100vh" class="detailPop">
							<div class="pop-title">
								...
								<div class="pop-right">
									...
									<div class="font-13 color-80 stocks" v-if="detailObj.sellType==1">
										库存：{{detailObj.stocks}}
									</div>
		...
		methods: {
			...
			plus: function(n) {
				...
				if(this.detailObj.isSecKill) {
					...
					if(this.detailObj.sellType==1 && this.detailObj.stocks <= this.countNum && n == 1) {
						this.$toast.show({
							text: '数量超出可售范围',
							position: 'middle',
							value: true
				...
				if(this.$store.state.loginUser["ucid"] != undefined) {
					...
					if(this.detailObj.isSecKill) {
						...
						Apis.getSecKillTimeList().then(data => {
							...
							//秒杀商品要判断是否已经购买过
							if(this.isCart) {
								Apis.getisByProd(this.$store.state.loginUser.token, {
									'prodId': this.detailObj.prod_id
								}).then(data => {
									console.log(data)
									if(data.code == 1) {
											...
											if(data.code == 1) {
												if(this.isCart) {
													this.$router.push({
														path: '/cart',
														query: {
															'isCash': this.detailObj.sellType
														}
													})
												} else {
													this.$router.push({
								...
								Apis.getisByProdInOrder(this.$store.state.loginUser.token, {
									'prodId': this.detailObj.prod_id
								}).then(data => {
									console.log(data)
									if(data.code == 1) {
			...
			init: function() {
				let _this = this
				this.$http.get(this.$Api('/home/getProdDetail'), {
					params: {
						'prodId': this.$route.query.prod_id
					}
				}).then((response) => {
					...
					this.$store.dispatch({
						type: 'setIsCash',
						data: this.detailObj.sellType == 0 ? true : false
					})
