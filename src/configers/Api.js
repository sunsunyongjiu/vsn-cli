...
export default {
	...
	get(params = {}) {
			return Vue.http.get(basePath + '/home/getCommendProdList', {
				params
			}).then(res => {
				// some handling
				return res.data
		...
		getServerTime(params = {}) {
			return Vue.http.get(basePath + '/home/getServerTime', {
				params
			}).then(res => {
				// some handling
				return res.data
		...
		// 登陆
		login(params = {}) {
			return Vue.http.post(basePath + '/login', params, {
				emulateJSON: true
			}).then(res => {
				// some handling
				return res.data
		...
		uploadImage(imageBase64, imageType) {
			...
			return Vue.http.post(basePath + '/order/uploadImage', params, {
				emulateJSON: true
			}).then(res => {
				// some handling
				return res.data
		...
		searchProdList(params = {}) {
			return Vue.http.get(basePath + '/home/searchProdList', {
				params
			}).then(res => {
				// some handling
				return res.data
		...
		getProdListByCategory(params = {}) {
			return Vue.http.get(basePath + '/home/getProdListByCategory', {
				params
			}).then(res => {
				// some handling
				return res.data
		...
		getSecKillProdList(params = {}) {
			return Vue.http.get(basePath + '/home/getSecKillProdList', {
				params
			}).then(res => {
				// some handling
				return res.data
...
function header(url, token) {
	return {
		"token": token,
		"time": timer,
		"sign": sha256(url + token + timer).toUpperCase(),
	}
}